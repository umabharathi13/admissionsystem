...
public class CollegeController {
	...
	public ResponseEntity<College> getCollegeById(@PathVariable("id") int collegeId) {
		if (collegeService.getByCollegeId(collegeId) == null) {
			throw new CollegeNotFoundException("College not found for id:" + collegeId);
		}
		logger.info("get college details by college Id");
		College college = collegeService.getByCollegeId(collegeId);
		return ResponseEntity.ok(college);
	}
	...
	public ResponseEntity<College> deleteCollegeById(@PathVariable("id") int collegeId) {
		if (collegeService.deleteByCollegeId(collegeId) == null) {
		...
		College college = collegeService.deleteByCollegeId(collegeId);
		return ResponseEntity.ok(college);
	}
	...
	public ResponseEntity<College> updateCollegeName(@PathVariable("id") int collegeId, @RequestBody College college) {
		if (collegeService.getByCollegeId(collegeId) == null) {
		...
		College college1 = collegeService.updateCollegeName(collegeId, college);
